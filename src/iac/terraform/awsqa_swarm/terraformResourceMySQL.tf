resource "aws_instance" "awsqa_swarm_mysql" {
  count           = 1
  ami             = "ami-759bc50a"
  instance_type   = "t2.small"
  key_name        = aws_key_pair.awsqa_swarm_key_pair.key_name
  security_groups = [aws_security_group.awsqa_swarm_mysql.name]
  tags = {
    Name = "AWSQA-SWARM MySQL Server ${format("%03d", count.index)}"
  }
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --region ${regex("[a-z]+[^a-z][a-z]+[^a-z][0-9]+",self.availability_zone)} --instance-ids ${aws_instance.awsqa_swarm_mysql[count.index].id}"
  }
  provisioner "file" {
    connection {
      type = "ssh"
